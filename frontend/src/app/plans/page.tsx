...
import { Check, Zap, Rocket, CreditCard, AlertTriangle } from 'lucide-react';
...
const plans = [
  ...
  },
  {
    ...
    name: 'Pro',
    price: '$29 / mes',
    description: 'Para centros en producción',
    features: [
...
export default function PlansPage() {
  ...
  const planMutation = useMutation({
    mutationFn: (status: 'trial' | 'active') =>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {plans.map((plan) => {
          ...
          const isCurrent = currentStatus === plan.targetStatus ||
            (plan.targetStatus === 'active' && currentStatus === 'active');
          const isUpgrade = plan.targetStatus === 'active' && currentStatus !== 'active';
          const isDowngrade = plan.targetStatus === 'trial' && currentStatus === 'active';
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${plan.highlighted ? 'bg-blue-100' : 'bg-gray-100'}`}>
              ...
              <div className="pt-2">
                {isCurrent ? (
                  ...
                  <Button
                    className="w-full"
                    onClick={() => planMutation.mutate('active')}
                    disabled={planMutation.isPending}
                  >
                    {planMutation.isPending ? 'Procesando...' : 'Contratar plan Pro'}
                  </Button>
                ) : isDowngrade ? (
                  <Button
                    variant="outline"
                    className="w-full"
                    onClick={() => planMutation.mutate('trial')}
                    disabled={planMutation.isPending}
                  >
                    {planMutation.isPending ? 'Procesando...' : 'Volver a Trial'}
                  </Button>
                ) : (
                  <Button
                    className="w-full"
                    onClick={() => planMutation.mutate(plan.targetStatus as 'trial' | 'active')}
                    disabled={planMutation.isPending}
                  >
                    {planMutation.isPending ? 'Procesando...' : 'Activar'}
                  </Button>
                )}
              </div>
            </div>
          ...
          <table className="w-full text-sm">
            <thead className="border-t border-b bg-gray-50">
              <tr>
                ...
                <th className="px-5 py-3 text-center font-medium text-gray-500 text-xs uppercase tracking-wide">Pro</th>
              </tr>
            </thead>
            <tbody className="divide-y text-gray-700">
              {[
                ['Servicios activos', '1', 'Ilimitados'],
                ['Reservas totales', '3', 'Ilimitadas'],
                ['Portal de reservas', '✓', '✓'],
                ['API keys', '1', 'Múltiples'],
