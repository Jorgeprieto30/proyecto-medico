...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="text-center space-y-4">
        ...
        <Link href="/portal/forgot-password" className="text-sm text-blue-600 hover:underline block">
  ...
  const onSubmit = async (data: Form) => {
    setServerError('');
    try {
      const res = await fetch(`${BASE}/members/reset-password`, {
        method: 'POST',
        ...
        {serverError && (
          <div className="text-sm text-red-600 bg-red-50 rounded-lg px-4 py-2.5">
            ...
            {serverError.includes('expirado') && (
              <Link href="/portal/forgot-password" className="underline font-medium">Solicitar nuevo enlace</Link>
            )}
          </div>
        )}
        <div>
